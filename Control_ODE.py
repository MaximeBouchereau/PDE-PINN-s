...
class NN(nn.Module):
    ...
    def forward(self, t):
        ...
        y, y_0, u = t, torch.zeros_like(t), t

        # Structure of the solution of the equation
        for i, module in enumerate(self.Y):
            y = module(y)
            y_0 = module(y_0)

        # Structure of the control of the equation
        for i, module in enumerate(self.U):
            u = module(u)

        y, u = (y - y_0 + torch.kron(self.y0(), torch.ones_like(t))).T, u.T
...
class ML(NN):
    ...
    def Loss_autograd(self, t, T, model):
        """Computes the Loss function associated with the PINN
        Inputs:
        - t: Tensor of shape (1,n): Inputs of Neural Network - time variable
        - T: Float - Final time of ODE control
        - model: Neural network which will be optimized for learning solution and control
        ...
        y_hat = torch.cat((torch.zeros_like(t), torch.zeros_like(t)), dim=0)
        y_hat.requires_grad = True

        u_hat = torch.zeros_like(t)
        ...
        loss_FC = ((y_hat_1 - self.y1().T) ** 2).mean() # Loss associated to final conditions (control)


        return loss_ODE + loss_FC, loss_ODE, loss_FC
    ...
    def Train_autograd(self, model , T = 1 , K = 1000 , BS = 64 ,  N_epochs = 100 , N_epochs_print = 10):
        ...
        for epoch in range(N_epochs + 1):
            for ixs in torch.split(torch.arange(t_train.shape[1]), BS):
                ...
                model.train()
                t_batch = t_train[:, ixs]
                loss_train = self.Loss_autograd(t_batch, T, model)[0]
                loss_train.backward()
                optimizer.step()  # Optimizer passes to the next epoch for gradient descent

            loss_train, loss_train_ODE, loss_train_FC = self.Loss_autograd(t_train, T, model)
            loss_test, loss_test_ODE, loss_test_FC = self.Loss_autograd(t_test, T, model)
            ...
            if epoch % N_epochs_print == 0:  # Print of Loss values (one print each N_epochs_print epochs)
                ...
                print('       > Train: ODE:', format(loss_train_ODE, '.2E'), '- FC:', format(loss_train_FC, '.2E'))
                print('       > Test: ODE:', format(loss_test_ODE, '.2E'), '- FC:', format(loss_test_FC, '.2E'))
    ...
    def Integrate(self, name_model="model_Control_ODE",  ht = 0.02, save_fig=False):
